...
class LandingPage extends Component {
  ...
  render = () => {
    ...
    return <section className={classes} style={{ maxHeight }}>
        <div>
          {newUser ? <h1>
              <span>S</span>
              <span>I</span>
          ...
          {loginErrors !== "Error:" || errors !== "" ? <h2>
              {loginErrors !== "Error:" ? loginErrors : errors}
            </h2> : ""}
          <div>
            {this.state.newUser && <div>
                <input onBlur={this.blur} onFocus={this.focus} type="text" id="name" ref="name" name="name" value={name} onChange={e => this.setState(
                      { name: e.target.value }
                    )} />
                <label htmlFor="name">Full Name</label>
              </div>}
            <div>
              <input onBlur={this.blur} onFocus={this.focus} type="email" id="email" ref="email" name="email" value={email} onChange={e => this.setState(
                    { email: e.target.value }
                  )} />
              <label htmlFor="email">Email</label>
            </div>
            <div>
              <input onBlur={this.blur} onFocus={this.focus} type="password" id="password" ref="password" name="password" value={password} onChange={e => this.setState(
            ...
            {newUser ? <h2>
                Already have an account? <a onClick={this.isNewUser}>
                  Sign In
                </a>
              </h2> : <h2>
